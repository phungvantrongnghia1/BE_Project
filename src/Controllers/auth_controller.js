...
module.exports.login =async (req,res) => {
    ...
    knex.from('user').select("*").where('Email', '=', req.body.Email)
        .then(async (rows) => {
            if (rows.length === 0) return res.status(401).send("Email or Password is not correct!")
            const validPassword = await brcypt.compare(req.body.Password, rows[0].Password)
            if (!validPassword) return res.status(400).send("Email or Password is not correct! smk")
            const token = jwt.sign({ id: 'user.id' }, process.env.TOKEN_SECRET) // lấy id của user vừa login tạo token
